...
private:
	...
	virtual std::vector<std::unique_ptr<geometry<D>>> analyse(
			std::vector<std::reference_wrapper<line_segment<D>>>&& line_segments) {
		...
		for (auto iter1 = line_segments.begin(); iter1 != line_segments.end();
			...
			for (auto iter2 = std::next(iter1); iter2 != line_segments.end();
					 ++iter2) {
				line_segment<D>& a = *iter1;
				line_segment<D>& b = *iter2;
